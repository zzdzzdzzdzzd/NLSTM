...
import com.zzd.model.nLstmNetwork.LstmNetwork;
...
public class MainForMyLstm {
	...
	public double[][] model1(String timeFlag, boolean isDraw){
		...
		for(int t=0;t<times;t++){
			LstmNetwork lstm=new LstmNetwork(step*columnIndex.length, 16);
			lstm.train(trainX, trainY, validateX, validateY, 32, lr, epochs);

			Map<String,double[]> result=lstm.validateOutput(trainX, trainY, validateX, validateY);
			...
			if(isDraw){
				MyChart.drawCompare(preMatrix.transpose().getArray(), labelMatrix.transpose().getArray(), "model 1");				
			}
		...
		String excelPath="resources/data/result/"+timeFlag+"/LSTM_AllFeatures_"+timeFlag+".xls";
		FileOperatorUtil.exportPredictions(contents, excelPath);
		return metrics;

	}
	
	public double[][] model2(String timeFlag, boolean isDraw){
		int[] rowIndex=new int[747];
		for(int i=1;i<=rowIndex.length;i++){
			rowIndex[i-1]=i;
		}
		...
		for(int t=0;t<times;t++){
			...
			Matrix labelMatrix = new Matrix(y, 1);
			if(isDraw){
				MyChart.drawCompare(preMatrix.transpose().getArray(), labelMatrix.transpose().getArray(), "model 2");				
			}
			metrics[t] = LossFunction.calLoss(preMatrix.transpose(), labelMatrix.transpose(), true);
			int len=preNorm.length;
			double[][] content=new double[len][4];
		...
		String excelPath="resources/data/result/"+timeFlag+"/LSTM_OnlyWindSpeed_"+timeFlag+".xls";
		FileOperatorUtil.exportPredictions(contents, excelPath);
		return metrics;
